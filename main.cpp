...
public:
    ...
    virtual vec3 vertex(int iface, int nthvert)
    {
        ...
        v_wc.set_col(nthvert, gl_vertex);
    ...
    virtual bool fragment(vec3 bar, TGAColor &color)
    {
        ...
        {                                                                                                         //TBN
            ...
            mat<3, 3> A;
            A[0] = v_wc.col(1) - v_wc.col(0);
            A[1] = v_wc.col(2) - v_wc.col(0);
            A[2] = b_n; //bn
        ...
        l = trans_vec3(u_view * u_model, (v_wc * bar - light_pos).normalize(), 0.0).normalize();
        double nl = -(l * n);
...
int main(const int argc, const char **argv)
{
    ...
    { //shadow mapping
        ...
        */
        ...
        if (image.write_tga_file(output_dir + "sbuffer.tga"))
            std::cout << "sbuffer output to " << output_dir + "sbuffer.tga" << std::endl;
        else
    ...
    { //rendering frame
        ...
        mat<4, 4> camera_view = get_view(eye_pos, light_pos, up);
        for (uint8_t cnt = 0; cnt < 2; cnt++)
        {
            std::cout << "process model #" << char(cnt + '0') << std::endl;
            load(cnt);
